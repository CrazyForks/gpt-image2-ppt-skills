...
很多人做 PPT 时会困惑："我有 10 段内容，是不是做 10 页？" 或者"老板说要做 8 页，那就把内容分成 8 段？" 这两种思路都容易做出流水账式的 PPT——每页信息密度差不多，观众看完记不住重点。

更好的做法是：**按"观众需要在哪些地方停顿理解"来拆页**，而不是机械地按段落数量或字数均分。

### 三个拆页原则

**1. 一页讲透一件事**

每页应该只承载一个完整的信息单元。问自己："如果把这页单独发给同事，他能不能看懂这页在讲什么？" 如果需要翻到上一页或下一页才能理解，说明拆得不够干净。

**2. 关键转折单独成页**

当内容出现"但是"、"然而"、"相比之下"、"问题在于"这类转折时，转折前后最好分成两页。对比、反差、意外结论是观众最容易记住的部分，给它们足够的视觉空间。

**3. 复杂流程先总后分**

如果一个流程有 5 个以上的步骤，不要塞进一页。可以先用一页展示完整流程（鸟瞰视角），再用 1-2 页展开关键步骤的细节。

### 常见的拆页场景

| 内容类型 | 推荐拆法 | 示例 |
|---------|---------|------|
| **问题 + 解决方案** | 2 页：问题页 + 方案页（你的产品/方法） | "市场现状：三大痛点" → "我们的解法" |
| **对比（A vs B）** | 1 页并排对比，或 2 页（先 A 再 B） | "传统方式 vs 我们的方式"放一页左右分栏 |
| **多步骤流程** | 步骤 ≤4 步放一页；≥5 步拆成"全流程页 + 关键步骤详解页" | 用户旅程 8 步 → 第 1 页鸟瞰 8 步，第 2 页放大第 3-4 步 |
| **数据堆叠** | 按对比维度拆页，每页 2-3 个数据 | "效率提升 3 倍 + 成本降低 40%" 一页，"用户满意度 90%" 另一页 |
| **案例故事** | 按"背景 → 冲突 → 解决 → 结果"拆成 2-3 页 | 第 1 页客户遇到的问题，第 2 页用你的方案后的变化 |
| **理论/方法论** | 先结论页（是什么），再原理页（为什么），最后应用页（怎么用） | "什么是增长飞轮" → "为什么有效" → "三步搭建你的飞轮" |

### 拆页实战示例

假设要把这段 300 字的产品介绍做成 PPT：

> 我们做了一个智能会议助手。现在公司开会有三个问题：会议记录靠人工，整理慢还容易漏；会上决定的事情散落在各个聊天记录里，后续跟进难；不同部门重复开类似的会，浪费时间。我们的产品用 AI 实时转录会议，自动提取待办事项并分配给相关人，还能把历史会议内容结构化存储，开会前推荐相关的历史决策。目前 500 强企业客户留存率 85%，平均每家企业减少 30% 的会议时长。
...
问题：第 2、3 页信息密度太高，观众记不住"到底解决了什么"。

**✅ 好的拆法（按关键信息点）**：
- 第 1 页：核心价值主张 —— "让每场会议产生可追溯的行动"（一句话点题）
- 第 2 页：最痛的痛点 —— "会上决定的事，会后没人记得要做"（聚焦一个最致命的场景，在 slides_plan.md 里写清楚要对比"开会时说了什么 / 一周后忘了什么"，AI 会据此设计视觉呈现）
- 第 3 页：产品闭环 —— "开会 → AI 提取待办 → 自动分配 → 跟进提醒"（流程图，4 步刚好）
- 第 4 页：意外数据 —— "客户平均减少 30% 会议时长"（不是"开更高效的会"，而是"少开会"，这是反直觉的，值得单独强调）

**为什么第二种更好**：
- 第 1 页直接给结论，不绕弯
- 第 2 页只讲一个痛点，但讲透了（用场景化的方式让观众共情）
- 第 3 页是完整闭环，观众能记住"4 步走"
- 第 4 页的数据有记忆点（不是常规的"效率提升"，而是"开会变少了"）

### Agent 该怎么帮用户拆页

当用户说"帮我做一份 XX 的 PPT"时：

1. **先别急着写大纲，先问 3 个问题**：
   - 这是给谁看的？（决定信息密度和专业术语的使用）
   - 讲多久？（5 分钟 = 6-8 页；15 分钟 = 12-15 页；超过 20 分钟建议拆成两份 PPT）
   - 你最希望观众记住什么？（倒推哪些页必须有）

2. **帮用户找"必须单独成页"的内容**：
   - 有没有反直觉的数据/结论？（单独成页，放大展示）
   - 有没有前后对比/新旧方案对比？（对比必须视觉化，单独成页）
   - 有没有超过 4 步的流程？（先画全景，再拆细节）

3. **写 `slides_plan.md` 时，每个 h2 标题自己问一遍**：
   - "这页的标题能不能单独传播？"（如果太平淡，改成更有张力的表达）
   - "这页和上一页之间信息密度差异够不够明显？"（如果内容连续且平滑，考虑合并）

4. **页数建议**：
   - 封面（必需）+ 4-6 页内容 + 1 页数据/总结 = 6-8 页（标准长度）
   - 如果拆完发现超过 12 页，问用户："要不要把一部分内容放到附录/备查页？"
   - 如果只有 3-4 页，检查是否漏了关键环节（比如"为什么是你来做这件事"这种可信度支撑页）
